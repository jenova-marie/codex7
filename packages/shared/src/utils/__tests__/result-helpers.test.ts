...
describe('wrapPromise', () => {
  it('should wrap successful promise in Ok', async () => {
    ...
    const result = await wrapPromise(
      promise,
      (e) => new ValidationError('test', String(e))
    );
  ...
  it('should map error correctly', async () => {
    ...
    const result = await wrapPromise(
      promise,
      (e) => new DatabaseQueryError('SELECT *', e as Error)
    );
...
describe('mapResults', () => {
  ...
  it('should handle empty array', async () => {
    const result = await mapResults([], async (n: number) => Ok(n));
...
describe('tryAsync', () => {
  ...
  it('should handle JSON.parse errors', async () => {
    const result = await tryAsync(
      async () => JSON.parse('invalid json'),
      (e) => new ValidationError('json', e instanceof Error ? e.message : String(e))
    );
